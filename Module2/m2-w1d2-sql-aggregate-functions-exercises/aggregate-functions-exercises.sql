...
-- (232 rows)
select (gnp*1000000/population) percapita from country where population > 0 order by percapita desc
...
select code from country where region like '%Australia%'
...
select max(surfacearea/population) from country where population >0

-- 15. The population density and life expectancy of the top ten countries with the 
-- highest life expectancies in descending order. 
-- (highest life expectancies in world: 83.5, 0.006, "Andorra")
select top 10 name, (surfacearea/population) as Density, lifeexpectancy from country order by lifeexpectancy desc

-- 16. The difference between the previous and current GNP of all the countries in 
-- the world ordered by the absolute value of the difference. Display both 
-- difference and absolute difference.
-- (smallest difference: 1.00, 1.00, "Ecuador")
select name, (gnp - gnpold), abs(gnp - gnpold) as absgnp from country where gnp is not null and gnpold is not null order by absgnp 
